...
interface EditVehiculoFormProps {
  vehiculoData: VehiculoWithRelations
  setOpenDialog: (open: boolean) => void
  onSuccess?: () => void
}

export function EditVehiculoForm({ vehiculoData, setOpenDialog, ...props }: EditVehiculoFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      noChasis: vehiculoData.noChasis,
      noMotor: vehiculoData.noMotor,
      noPlaca: vehiculoData.noPlaca,
      tpVehiculoId: vehiculoData.tpVehiculoId,
      marcaId: vehiculoData.marcaId,
      modeloId: vehiculoData.modeloId,
      tpCombustibleId: vehiculoData.tpCombustibleId,
      photo: vehiculoData.photo,
  ...
  useEffect(() => {
    if (marcaId) {
      ...
      // No resetear el modelo si estamos editando y el modelo pertenece a la marca
      if (!modelosFiltrados.some((m) => m.id === form.getValues("modeloId"))) {
  ...
  const loadSelectData = async () => {
    try {
      ...
      if (modelosResponse.ok) {
        ...
        // Inicializar modelos filtrados
        if (vehiculoData.marcaId) {
          setFilteredModelos(modelosData.filter((modelo: Modelo) => modelo.idMarca === vehiculoData.marcaId))
        }
      }
      ...
      if (combustiblesResponse.ok) {
        ...
        setTiposCombustible(combustiblesData)
      }
    } catch (error) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setOpenDialog(false)
    try {
      await axios.patch(`/api/vehiculos/${vehiculoData.id}`, values)
      toast({
      ...
      toast({
        ...
        description: "No se pudo modificar el vehículo",
        variant: "destructive",
      })
    }
  }
