...
export async function PATCH(req: Request, { params }: { params: { marcaId: string } }) {
  try {
    const { userId } = await auth()
    const { marcaId } = params
    ...
    console.log("[MARCA_ID]", error)
    return new NextResponse("Internal Error", { status: 500 })
  }
}

export async function DELETE(req: Request, { params }: { params: { marcaId: string } }) {
  try {
    const { userId } = await auth()
    const { marcaId } = params
